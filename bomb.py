  ...
  def __init__(self, parent=None):
    ...
    for r in range(self.rnum):
      ...
      for c in range(self.cnum):
        ...
        self.button[r][c].setFixedSize(30,30)
    ...
    self.timelabel.setStyleSheet("background-color: rgb(0, 0, 0); color: rgb(32,70,128)")
    self.newGame()
    ...
    biggamebutton = QtGui.QCheckBox("Big Game", self)
    biggamebutton.move(10,0)
    biggamebutton.stateChanged.connect(self.setRowandCol)

#-----------------------------------------------------
#-----------------------------------------------------
  def setRowandCol(self, biggamebutton):
    if biggamebutton == QtCore.Qt.Checked:
      self.rnum = 32
      self.cnum = 32   
    else:
      self.rnum = 16
      self.cnum = 16
    print(self.rnum, self.cnum)
#-----------------------------------------------------
  def newGame(self):
    self.populateBombs()
    self.marked = 0
    ...
    for r in range(self.rnum):
      for c in range(self.cnum):
        ...
        self.button[r][c].setStyleSheet("background-color: rgb(32,70,128); color: rgb(32,70,128)")
...
#-----------------------------------------------------
  def grepSurroundingLocation(self, row, col):
    ...
    if top > -1 and right < self.cnum:
      ret.append( [top, right] )  # UpperRight
    if bot < self.cnum and left > -1:
      ret.append( [bot, left] )   # LowerLeft
    if bot < self.rnum and right < self.rnum:
      ret.append( [bot, right] )  # LowerRight
    return(ret)
#-----------------------------------------------------
  def populateBombs(self):
    bomblocation = random.sample(range(self.rnum * self.cnum), 40)
    x = 0
    for r in range(self.rnum):
      for c in range(self.cnum):
